...
function getPreviewStyle(theme: Theme) {
    const colors = previewColors[theme.slug] ?? { bg: '#1e293b', accent: '#6366f1' };
    return {
...
function activate(theme: Theme) {
    router.post(admin.themes.activate.url({ theme: theme.id }), {}, { preserveScroll: true });
}
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div v-if="props.themes.length === 0" class="flex flex-col items-center justify-center rounded-lg border border-dashed py-16 text-center">
            <Paintbrush class="mb-3 h-10 w-10 text-muted-foreground/40" />
            <p class="text-sm text-muted-foreground">No themes found.</p>
            <p class="mt-1 text-xs text-muted-foreground">Add a theme folder under <code class="rounded bg-muted px-1">resources/themes/</code>.</p>
        </div>
        ...
        <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
                ...
                <div class="h-32 w-full" :style="getPreviewStyle(theme)">
                    <div class="flex h-full flex-col items-start justify-end p-4">
                        <span
                            class="rounded px-2 py-0.5 text-xs font-semibold"
                            :style="{ background: previewColors[theme.slug]?.accent ?? '#6366f1', color: '#fff' }"
                        >
                            {{ theme.name }}
                        </span>
                    </div>
                ...
                <CardHeader class="pb-2">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <Badge v-if="theme.is_active" variant="default" class="shrink-0">
                            <CheckCircle2 class="mr-1 h-3 w-3" />
                            Active
                        </Badge>
                    </div>
                    <CardDescription v-if="theme.description">{{ theme.description }}</CardDescription>
                </CardHeader>
                ...
                <CardContent class="pb-2 text-xs text-muted-foreground">
                    ...
                    <span v-if="!theme.version && !theme.author" class="italic">No metadata</span>
                </CardContent>
                ...
                <CardFooter>
                    ...
                    <p v-else class="w-full text-center text-xs text-muted-foreground">Currently active</p>
                </CardFooter>
            </Card>
        </div>
    </AppLayout>
