    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex items-center justify-between">
            ...
            <Badge :variant="isDown ? 'destructive' : 'default'">
                <component
                    :is="isDown ? AlertTriangle : CheckCircle"
            ...
            <CardHeader>
                <div class="flex items-center gap-3">
                    <div
                        ...
                        :class="isDown ? 'bg-destructive/10' : 'bg-muted'"
                    >
                        <WrenchIcon
                            class="h-5 w-5"
                            :class="
                    ...
                    <div>
                        ...
                        <CardDescription v-if="isDown && message"
                            >Message: {{ message }}</CardDescription
                        >
                    </div>
                </div>
            </CardHeader>
        ...
        <Card v-if="!isDown">
            <CardHeader>
                ...
                <CardDescription>
                    Visitors will see a 503 maintenance page. You can access the
                    site via the secret URL bypass.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <form
                    @submit.prevent="
                        enableForm.post(admin.maintenance.enable.url())
                    "
                    class="space-y-4"
                >
                    <div class="space-y-1.5">
                        <Label for="maint-message"
                            >Message
                        ...
                        />
                        <p
                            v-if="enableForm.errors.message"
                            class="text-xs text-destructive"
                        >
                            {{ enableForm.errors.message }}
                        </p>
                    </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <p class="text-xs text-muted-foreground">
                                ...
                                <code class="font-mono"
                                    >/{{
                                        enableForm.secret || 'your-secret'
                                    }}</code
                                >
                            </p>
                        </div>
                        <div class="space-y-1.5">
                            <Label for="maint-retry"
                                >Retry After (seconds)</Label
                            >
                            <Input
                                id="maint-retry"
                            ...
                            <p class="text-xs text-muted-foreground">
                                Sent as
                                <code class="font-mono">Retry-After</code> HTTP
                                header
                            </p>
                        </div>
                    </div>
