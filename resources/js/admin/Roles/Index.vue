...
import { ChevronDown, ChevronRight, Plus, ShieldCheck, Trash2 } from 'lucide-vue-next';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Roles & Permissions', href: admin.roles.index.url() },
];
...
function toggleGroupPerms(groupPerms: string[], checked: boolean) {
    if (checked) {
        ...
        editPerms.value = editPerms.value.filter((p) => !groupPerms.includes(p));
    }
}
...
function createRole() {
    router.post(
        ...
        { preserveScroll: true, onSuccess: () => { creating.value = false; } },
    );
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="space-y-3">
            <Card v-for="role in props.roles" :key="role.id">
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <Badge :variant="roleVariant(role.name)" class="text-xs capitalize">
                                    {{ role.users_count }} user{{ role.users_count === 1 ? '' : 's' }}
                                </Badge>
                                <Badge v-if="BUILTIN.includes(role.name)" variant="outline" class="text-xs">
                                    built-in
                                </Badge>
                            </div>
                        </div>
                ...
                <CardContent v-else class="pb-3 pt-0 text-xs text-muted-foreground italic">
        ...
        <Dialog :open="!!editing" @update:open="(v) => { if (!v) editing = null; }">
            <DialogContent class="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle class="capitalize">Edit — {{ editing?.name }}</DialogTitle>
                    ...
                    <div v-for="([group, perms]) in permissionGroups" :key="group" class="rounded-md border">
                        ...
                        >
                            <Checkbox
                                ...
                                @update:checked="(v) => toggleGroupPerms(perms, !!v)"
                                @click.stop
                            />
                        ...
                        <div v-if="expandedGroups.has(group)" class="divide-y border-t">
                            <div v-for="perm in perms" :key="perm" class="flex items-center gap-3 px-3 py-2 pl-9">
                                ...
                                />
                                <Label :for="`perm-${perm}`" class="cursor-pointer text-sm">{{ perm }}</Label>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Create role dialog -->
        <Dialog :open="creating" @update:open="(v) => { if (!v) creating = false; }">
            <DialogContent class="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader>
                ...
                <div class="space-y-4 py-2">
                    <div class="space-y-1.5">
                        ...
                        <Input id="role-name" v-model="newName" placeholder="e.g. moderator" />
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div v-for="([group, perms]) in permissionGroups" :key="group" class="rounded-md border">
                            ...
                            >
                                <Checkbox
                                    :id="`new-group-${group}`"
                                    :checked="perms.every((p) => newPerms.includes(p)) || (perms.some((p) => newPerms.includes(p)) ? 'indeterminate' : false)"
                                    @update:checked="(v) => v ? newPerms.push(...perms.filter((p) => !newPerms.includes(p))) : (newPerms = newPerms.filter((p) => !perms.includes(p)))"
                                    @click.stop
                                />
                                <span class="flex-1 text-left capitalize">{{ group }}</span>
                                <ChevronDown v-if="expandedGroups.has(group)" class="h-4 w-4 text-muted-foreground" />
                            ...
                            <div v-if="expandedGroups.has(group)" class="divide-y border-t">
                                <div v-for="perm in perms" :key="perm" class="flex items-center gap-3 px-3 py-2 pl-9">
                                    <Checkbox
                                        ...
                                        @update:checked="(v) => v ? newPerms.push(perm) : (newPerms = newPerms.filter((p) => p !== perm))"
                                    />
                                    <Label :for="`new-perm-${perm}`" class="cursor-pointer text-sm">{{ perm }}</Label>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <DialogFooter>
                    ...
                    <Button :disabled="!newName.trim()" @click="createRole">Create</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    </AppLayout>
