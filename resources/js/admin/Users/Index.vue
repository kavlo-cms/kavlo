...
function saveUserRoles() {
    ...
    router.put(
        admin.users.updateRoles.url({ user: editingUser.value.id }),
...
function saveUserPerms() {
    ...
    router.put(
        admin.users.updatePermissions.url({ user: editingUserPerms.value.id }),
        { permissions: selectedUserPerms.value },
        {
            preserveScroll: true,
            onSuccess: () => {
                editingUserPerms.value = null;
            },
        },
    );
}
function groupUserPermsState(groupPerms: string[]): boolean | 'indeterminate' {
    const n = groupPerms.filter((p) =>
        selectedUserPerms.value.includes(p),
    ).length;
    if (n === 0) return false;
...
function toggleGroupUserPerms(groupPerms: string[], checked: boolean) {
    if (checked)
        selectedUserPerms.value.push(
            ...groupPerms.filter((p) => !selectedUserPerms.value.includes(p)),
        );
    else
        selectedUserPerms.value = selectedUserPerms.value.filter(
...
function createUser() {
    router.post(admin.users.store.url(), newUser.value, {
        preserveScroll: true,
        onSuccess: () => {
...
function toggleGroupPerms(groupPerms: string[], checked: boolean) {
    if (checked)
        editPerms.value.push(
            ...groupPerms.filter((p) => !editPerms.value.includes(p)),
        );
    else
...
function saveRolePermissions() {
    ...
    router.put(
        admin.roles.update.url({ role: String(editingRole.value.id) }),
...
function createRole() {
    router.post(admin.roles.store.url(), newRole.value, {
        preserveScroll: true,
        onSuccess: () => {
...
function destroyPermission(p: Permission) {
    if (
        !confirm(
            `Delete permission "${p.name}"? Roles and users with this permission will lose it.`,
        )
    )
        return;
    router.delete(admin.permissions.destroy.url({ permission: String(p.id) }), {
        preserveScroll: true,
    });
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <Tabs default-value="users">
            ...
            <TabsContent value="users" class="space-y-3 pt-4">
                <div class="flex justify-end">
                    <Button size="sm" @click="openCreateUser"
                        ><Plus class="mr-1.5 h-4 w-4" />New User</Button
                    >
                </div>
                <div
                ...
                <Card v-for="user in props.users" :key="user.id">
                    ...
                    >
                        ...
                        <div class="flex flex-wrap gap-1.5">
                            <Badge
                                v-for="perm in user.direct_permissions"
                                :key="perm"
                                variant="outline"
            ...
            <TabsContent value="roles" class="space-y-3 pt-4">
                <div class="flex justify-end">
                    <Button size="sm" @click="openCreateRole"
                        ><Plus class="mr-1.5 h-4 w-4" />New Role</Button
                    >
                </div>
                <Card v-for="role in props.allRoles" :key="role.id">
                    ...
                    <CardContent
                        ...
                        >No permissions assigned.</CardContent
                    >
                </Card>
            </TabsContent>
            ...
            <TabsContent value="permissions" class="space-y-3 pt-4">
                <div class="flex justify-end">
                    <Button size="sm" @click="openCreatePermission"
                        ><Plus class="mr-1.5 h-4 w-4" />New Permission</Button
                    >
                </div>
                <Card>
        ...
        >
            <DialogContent class="sm:max-w-sm">
                ...
                <DialogFooter>
                    <Button variant="outline" @click="editingUser = null"
                        >Cancel</Button
                    >
        ...
        <Dialog
            :open="!!editingUserPerms"
            @update:open="
                (v) => {
                    if (!v) editingUserPerms = null;
                }
            "
        >
            <DialogContent class="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle
                        >Direct Permissions —
                        {{ editingUserPerms?.name }}</DialogTitle
                    >
                    <DialogDescription
                        >These are granted in addition to any permissions from
                ...
                <div class="space-y-3 py-2">
                    ...
                    >
                        ...
                        >
                            <Checkbox
                                :id="`upg-${group}`"
                                :checked="groupUserPermsState(perms)"
                                @update:checked="
                                    (v: boolean) =>
                                        toggleGroupUserPerms(perms, v)
                                "
                                @click.stop
                            />
                        ...
                        >
                            ...
                            >
                                <Checkbox
                                    :id="`upp-${perm}`"
                                    :checked="selectedUserPerms.includes(perm)"
                                    @update:checked="
                                        (v: boolean) =>
                                            v
                                                ? selectedUserPerms.push(perm)
                                                : selectedUserPerms.splice(
                                                      selectedUserPerms.indexOf(
                                                          perm,
                                                      ),
                                                      1,
                                                  )
                ...
                <DialogFooter>
                    <Button variant="outline" @click="editingUserPerms = null"
                        >Cancel</Button
                    >
        ...
        >
            <DialogContent class="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription
                        >Create a new user and assign their
                ...
                <div class="space-y-4 py-2">
                    ...
                    <div class="space-y-1.5">
                        <Label for="new-pw2">Confirm Password</Label
                        ><Input
                            id="new-pw2"
                ...
                <DialogFooter>
                    <Button variant="outline" @click="creatingUser = false"
                        >Cancel</Button
                    >
                    <Button
                        :disabled="
            ...
            <DialogContent class="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle class="capitalize"
                        >Edit permissions — {{ editingRole?.name }}</DialogTitle
                    >
                    <DialogDescription
                ...
                <DialogFooter>
                    <Button variant="outline" @click="editingRole = null"
                        >Cancel</Button
                    >
        ...
        >
            <DialogContent class="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                ...
                <DialogFooter>
                    <Button variant="outline" @click="creatingRole = false"
                        >Cancel</Button
                    >
                    <Button :disabled="!newRole.name.trim()" @click="createRole"
                        >Create Role</Button
                    >
                </DialogFooter>
            </DialogContent>
        ...
        <Dialog
            :open="creatingPermission"
            @update:open="
                (v) => {
                    if (!v) creatingPermission = false;
                }
            "
        >
            <DialogContent class="sm:max-w-sm">
                <DialogHeader>
                    ...
                    <DialogDescription
                        >Use lowercase words separated by spaces, e.g.
                        <code class="rounded bg-muted px-1">publish posts</code
                        >.</DialogDescription
                    >
                </DialogHeader>
                ...
                <DialogFooter>
                    <Button
                        variant="outline"
                        @click="creatingPermission = false"
                        >Cancel</Button
                    >
                    <Button
