...
const blockSchema = computed(
    () =>
        props.availableBlocks.find(
            (block) => block.type === props.selectedBlock?.type,
        ) ?? null,
);
const selectedContext = computed(
    () =>
        props.availableContexts.find(
            (context) => context.key === props.form.context_key,
        ) ??
        props.availableContexts[0] ??
        null,
);
...
<template>
    ...
    >
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        These variables can be inserted anywhere in the subject
                        or body blocks.
                    </p>
                </div>

                <div
                    v-if="selectedContext?.variables?.length"
                    class="space-y-2"
                >
                    <div
                        v-for="variable in selectedContext.variables"
                        :key="variable.key"
                ...
                <p v-else class="text-xs text-muted-foreground">
                    No variables registered for this template context yet.
                </p>
            </div>
        </div>
        ...
        >
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-sm font-medium">
                        {{ blockSchema?.label ?? selectedBlock.type }}
                    </p>
                    <p
                        v-if="blockSchema?.description"
                        class="text-xs text-muted-foreground"
                    >
            ...
            <template
                v-for="field in blockSchema?.fields ?? []"
                :key="field.key"
            >
                <div class="flex flex-col gap-1.5">
