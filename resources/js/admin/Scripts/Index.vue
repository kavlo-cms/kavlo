...
const sourceTypeLabel = (value: string) =>
    props.sourceTypeOptions.find((option) => option.value === value)?.label ??
    value;
const loadStrategyLabel = (value: string) =>
    props.loadStrategyOptions.find((option) => option.value === value)?.label ??
    value;
...
function updateSelectField(
    field: 'placement' | 'source_type' | 'load_strategy',
    value: unknown,
) {
        ...
        <div class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        These scripts are rendered on the public site according
                        to their placement and order.
                    </CardDescription>
                </CardHeader>
                <CardContent class="space-y-4">
                    ...
                    >
                        No managed scripts yet. Create one from the form on the
                        right.
                    </div>
                            ...
                            >
                                <div class="space-y-2">
                                    ...
                                    >
                                        ...
                                        <Badge variant="outline">{{
                                            sourceTypeLabel(script.source_type)
                                        }}</Badge>
                                        <Badge variant="outline">{{
                                            loadStrategyLabel(
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Choose where the script should load and whether it comes
                        from inline code, a URL, or an uploaded file.
                    </CardDescription>
                </CardHeader>
                <CardContent class="space-y-4">
                    <div class="space-y-2">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <Select
                                ...
                                @update:model-value="
                                    (value) =>
                                        updateSelectField('placement', value)
                                "
                            >
                                ...
                                <SelectContent>
                                    <SelectItem
                                        v-for="option in props.placementOptions"
                                        :key="option.value"
                                        :value="option.value"
                                    >
                                        {{ option.label }}
                        ...
                        <div class="space-y-2">
                            ...
                            <Select
                                ...
                                @update:model-value="
                                    (value) =>
                                        updateSelectField('source_type', value)
                                "
                            >
                                ...
                                <SelectContent>
                                    <SelectItem
                                        v-for="option in props.sourceTypeOptions"
                                        :key="option.value"
                                        :value="option.value"
                                    >
                                        {{ option.label }}
                    ...
                    <div v-if="form.source_type === 'upload'" class="space-y-2">
                        ...
                        <p class="text-xs text-muted-foreground">
                            ...
                            Existing uploaded files stay in place until you
                            replace them.
                        </p>
                        <p
                            v-if="selectedScript?.file_url"
                    ...
                    <div v-if="form.source_type === 'inline'" class="space-y-2">
                        ...
                        <p
                            v-if="form.errors.inline_content"
                            class="text-sm text-destructive"
                        >
                            {{ form.errors.inline_content }}
                        </p>
                    </div>

                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="space-y-2">
                            <Label for="script-load-strategy"
                                >Load Strategy</Label
                            >
                            <Select
                                :model-value="form.load_strategy"
                                @update:model-value="
                                    (value) =>
                                        updateSelectField(
                                            'load_strategy',
                            ...
                            >
                                <SelectTrigger id="script-load-strategy">
                                    <SelectValue
                                        placeholder="Select load strategy"
                                    />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem
                                        v-for="option in props.loadStrategyOptions"
                                        :key="option.value"
                                        :value="option.value"
                                    >
                                        {{ option.label }}
                            ...
                            <p
                                v-if="form.errors.load_strategy"
                                class="text-sm text-destructive"
                            >
                                {{ form.errors.load_strategy }}
                            </p>
                        </div>
